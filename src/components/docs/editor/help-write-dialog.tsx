  ...
  const generate = async () => {
    ...
    try {
      const res = await fetch("/api/ai/write", {
        ...
        body: JSON.stringify({ prompt: p, title: docTitle }),
      })
      ...
      <DialogContent className="flex flex-col gap-4 overflow-hidden p-4 sm:max-w-xl sm:p-6">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Sparkles className="h-4.5 w-4.5 text-primary" /> Help me write
          </DialogTitle>
          <DialogDescription className="text-left">
            Describe what you need and the AI drafts it for you. Edit the result, then insert it into your document.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="slim-scroll -mr-2 min-h-0 flex-1 space-y-3 overflow-y-auto pr-2">
          <Textarea
            ...
            placeholder="e.g. Write a project update for our stakeholders about the Q3 launch…"
          ...
          {result && (
            <div className="space-y-2">
              <p className="text-xs font-medium text-muted-foreground">Draft (editable)</p>
              <Textarea
                value={result}
        ...
        <DialogFooter className="text-xs text-muted-foreground">
          Generated by Z-AI · always review AI drafts
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
