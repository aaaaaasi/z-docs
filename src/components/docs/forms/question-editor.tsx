    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
        <Input
          ...
          placeholder={t("Question {n}", { n: index + 1 })}
          aria-label={t("Question {n} title", { n: index + 1 })}
        ...
        <Select value={question.type} onValueChange={(v) => changeType(v as QuestionType)}>
          <SelectTrigger
            aria-label={t("Question {n} type", { n: index + 1 })}
          ...
          <SelectContent>
            {QUESTION_TYPES.map((qt) => (
              <SelectItem key={qt.value} value={qt.value} className="text-[13px]">
                {t(qt.label)}
              </SelectItem>
            ))}
          </SelectContent>
        ...
        <div className="flex items-center gap-2">
          <Switch
            ...
            aria-label={t("Question {n} required", { n: index + 1 })}
          />
          <Label
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              ...
              aria-label={t("More actions for question {n}", { n: index + 1 })}
            >
          ...
          <DropdownMenuContent align="end" className="w-48">
            <DropdownMenuItem onClick={onDuplicate}>
              <Copy className="h-4 w-4" /> {t("Duplicate question")}
            </DropdownMenuItem>
      ...
      <div className="mt-4">
        ...
        {question.type === "rating" && (
          <div className="flex items-center gap-2.5 text-[13px] text-muted-foreground">
            ...
            <span>{t("Rating · 1–5 stars")}</span>
          </div>
        )}
        {question.type === "short" && (
          <Input disabled placeholder={t("Short-answer text")} aria-hidden="true" className="max-w-xs text-sm text-muted-foreground" />
        )}
